...
const Chat: React.FC<any> = (props) => {
  const {
    user: {
      userInfo: { _id, user_avatar, nickname: from_user_nickname },
    },
    message: { message_list, message_user_list },
  }: { user: userType; message: messageType } = useSelector(
    (store: any) => store,
  );
  ...
  const updateRead = async(cb: () => void) => {
    const idx = findObjIdxFromArr(message_user_list, {
      belong_user_id: _id,
      user_id: chat_id,
    });
        ...
        .map((item, index) => {
          ...
          return (
            <Item
              ...
              className={isMe ? 'chat-me' : 'chat-you'}
            >
              <div className="arrow_box">{item.content}</div>
            </Item>
          );
        })}
    </>
  ...
  const sendMsg = () => {
    ...
    socketObj.send({
      ...
      from_user_id: _id,
      from_user_nickname,
      to_user_avatar: avatar,
      to_user_id: chat_id,
      to_user_nickname: nickname,
      content: iptEle.current?.value,
    });
